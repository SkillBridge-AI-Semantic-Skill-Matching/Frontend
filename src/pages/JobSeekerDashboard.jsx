            ...
            <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col justify-between">
              <div>
                ...
                <div className="text-5xl font-bold text-indigo-700 mb-6">
                  {matches.length > 0 ? Math.max(...matches.map(m => Math.round(m.score || 0))) : 0}<span className="text-2xl text-indigo-400">%</span>
                </div>
                <div className="space-y-2 mb-6">
                  <div className="flex justify-between text-sm font-medium">
                    <span className="text-slate-700">Highest Alignment</span>
                    <span className="text-slate-900 font-bold">
                      {matches.length > 0 && Math.max(...matches.map(m => Math.round(m.score || 0))) > 80 ? 'High' : 
                       matches.length > 0 && Math.max(...matches.map(m => Math.round(m.score || 0))) > 50 ? 'Medium' : 'Low'}
                    </span>
                  </div>
                  <div className="h-2.5 w-full bg-slate-100 rounded-full overflow-hidden">
                    <div className="h-full bg-indigo-600 rounded-full" style={{ width: `${matches.length > 0 ? Math.max(...matches.map(m => Math.round(m.score || 0))) : 0}%` }}></div>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {matches.slice(0, 3).map((job) => (
                  <div key={job.id} className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col justify-between">
                    ...
                    <button onClick={() => { setSelectedJob(job); navigate('/dashboard/job_detail'); }} className="w-full py-2.5 bg-slate-50 hover:bg-indigo-50 text-slate-700 hover:text-indigo-700 font-bold text-sm rounded-xl transition-colors">
                      View Job
                    </button>
                  </div>
                ))}
